...
void kmeans_sequential(int numthreads, int N, int K, int* data_points, int** data_point_cluster_id, float** iter_centroids, int* num_iterations);
...
int number_of_threads_global=1;
...
int main(){
	...
	kmeans_sequential(num_threads,N, K, data_points, &cluster_points, &iter_centroids, &number_of_iterations);
...
void kmeans_sequential_execution(int *tid)
{
...
void kmeans_sequential(int numthreads,int N, int K, int* data_points, int** data_points_cluster_id, float** iter_centroids, int* num_iterations)
{

    // Initialize global variables
    number_of_points_global = N;
    number_of_threads_global = numthreads;
    ...
    *data_points_cluster_id = (int *)malloc(N * 3 * sizeof(int));
    ...
    */
    ...
    #pragma omp parallel
    {
        ...
        kmeans_sequential_execution(&ID);
    }
