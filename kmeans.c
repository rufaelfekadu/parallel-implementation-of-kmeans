...
int main(int argc, char** argv) {
  ...
  sites = (float *)malloc(sites_per_proc * d * sizeof(float));
  ...
  if (rank == 0) {
    ...
    grand_sums = (float *)malloc(k * d * sizeof(float));
    // assert(grand_counts = malloc(k * sizeof(int));
    grand_counts = (int *)malloc(k * sizeof(int));
  ...
  if ((rank == 0) && 1) {
    ...
    for (int i = 0;
	 ...
	 i++, site += d) {
      for (int j = 0; j < d; j++) fprintf(fout,"%f ", site[j]);
      // printf("%4d\n", all_labels[i]);
      fprintf(fout, "%4d\n",all_labels[i]);
    }
    fclose(fout);
  ...
  //   for(int i = 0; i < nprocs * sites_per_proc; i++){
